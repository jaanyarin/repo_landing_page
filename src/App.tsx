  ...
  return (
    <div className="site-shell">
      <header className="header">
        <a className="brand" href="#inicio" aria-label="Ir al inicio"><span className="brand-mark">AA</span><span><strong>Alejandro Anyarín</strong><small>Transformación Digital · Power Platform</small></span></a>
        <nav className={menuOpen ? 'nav open' : 'nav'}>{['inicio', 'servicios', 'proyectos', 'tecnologias', 'sobre-mi', 'contacto'].map(item => <a key={item} href={`#${item}`} onClick={() => setMenuOpen(false)}>{item.replace('-', ' ')}</a>)}</nav>
      ...
      <main>
        <section id="inicio" className="hero">
          <div className="hero-content"><span className="eyebrow"><Zap size={16}/> Soluciones empresariales</span><motion.h1 initial={{opacity:0,y:18}} animate={{opacity:1,y:0}}>Digitalizo procesos empresariales con <span>Power Apps</span> y <em>Power Automate</em></motion.h1><p>Desarrollo aplicaciones, automatizaciones y dashboards que mejoran el control operativo, reducen errores y convierten información dispersa en decisiones claras.</p><div className="hero-actions"><a className="button button-primary" href="#proyectos">Ver mis proyectos <ArrowRight size={18}/></a><a className="button button-secondary" href="#contacto">Solicitar una consulta</a></div><div className="trust-row"><span><CheckCircle2/> Soluciones escalables</span><span><CheckCircle2/> Enfoque empresarial</span><span><CheckCircle2/> Diseño responsive</span></div></div>
          <div className="hero-visual" aria-label="Vista conceptual de una solución Power Platform"><div className="glow"></div><div className="dashboard-window"><div className="window-bar"><i></i><i></i><i></i><span>Panel de operaciones</span></div><div className="dashboard-body"><aside><b>AA</b><span></span><span></span><span></span><span></span></aside><div className="dashboard-main"><div className="mini-stats"><div><small>Registros</small><strong>1,248</strong></div><div><small>Cumplimiento</small><strong>96%</strong></div><div><small>Alertas</small><strong>08</strong></div></div><div className="chart"><div className="bars"><i></i><i></i><i></i><i></i><i></i><i></i></div></div><div className="table-lines"><span></span><span></span><span></span><span></span></div></div></div></div><div className="phone-card"><div className="phone-notch"></div><strong>Nuevo registro</strong><label>Proceso<input value="Control operativo" readOnly/></label><label>Estado<input value="Completado" readOnly/></label><button>Guardar</button></div></div>
        </section>
        ...
        <section id="proyectos" className="section projects-section"><div className="section-heading"><div><span className="section-kicker">Portafolio</span><h2>Proyectos que convierten procesos en soluciones digitales</h2></div><p>Casos presentados con información anonimizada para proteger datos empresariales.</p></div><div className="projects-grid">{projects.map(project => <article className="project-card" key={project.title}><div className={`project-cover ${project.accent}`}><span>{project.category}</span><div className="mock-screen"><b>{project.title.split(' ').slice(0,3).join(' ')}</b><i></i><i></i><i></i></div></div><div className="project-body"><div className="tags">{project.tags.map(tag => <span key={tag}>{tag}</span>)}</div><h3>{project.title}</h3><p>{project.text}</p><a href="#contacto">Ver caso de estudio <ArrowRight size={16}/></a></div></article>)}</div></section>
